...
| Declaration | `class ShareInfoEventArgs` with `EventArgs` |
...
`ShareInfoEventArgs` is a `class` whose source-local responsibility is defined by the declaration, its members, and the collaborators listed below. This reference does not infer behavior beyond those source facts.

## Why it exists

Its declared base/contract list is `EventArgs`. The implementation text directly references **event subscription**.
