...
| Declaration | `class ZoomRoomFarEndCamera` with `ZoomRoomCamera, IAmFarEndCamera` |
...
`ZoomRoomFarEndCamera` is a `class` whose source-local responsibility is defined by the declaration, its members, and the collaborators listed below. This reference does not infer behavior beyond those source facts.

## Why it exists

Its declared base/contract list is `ZoomRoomCamera, IAmFarEndCamera`.
